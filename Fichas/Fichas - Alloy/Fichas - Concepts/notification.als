...
// o carater " ' " serve para indicar uma mudança de estado na entidade 
pred stutter {
	subscriptions' = subscriptions	
	notifications' = notifications
}

// esvaziamos a lista de notificações
pred read [u : User] {
...
// todos os users recebem notificação do evento em questão
pred occur [e : Event] {
...
run Scenario3 {
	...
	one u1 : User, u2 : User | some e : Event {
		subscribe[u1, e];
		subscribe[u2, e];
		occur[e]
	}
...
run Scenario4 {
	...
	some u : User | some disj e1, e2 : Event {
		subscribe[u, e1];
		subscribe[u, e2];
		occur[e1];
		occur[e2];
